(function(){
let numbers=[1,2,3];
report(query().select().from(numbers).execute(),JSON.stringify([1,2,3]))
report(query().select().execute(),'[]')
report(query().from(numbers).execute(),'[1,2,3]')
report(query().execute(),'[]')
report(query().from(numbers).select().execute(),'[1,2,3]')
...
report(query().select(profession).from(persons).where(isTeacher).execute(),'["teacher","teacher","teacher"]')
//["teacher","teacher","teacher"]);


//SELECT*FROMpersonsWHEREprofession="teacher"
report(query().from(persons).where(isTeacher).execute(),JSON.stringify(persons.slice(0,3)))
//persons.slice(0,3));
...
report(query().select(name).from(persons).where(isTeacher).execute(),'["Peter","Michael","Peter"]')
//["Peter","Michael","Peter"]);
report(query().where(isTeacher).from(persons).select(name).execute(),'["Peter","Michael","Peter"]')
//,["Peter","Michael","Peter"]);
...
report(query().select(professionGroup).from(persons).groupBy(profession).execute(),'["teacher","scientific","politician"]')
...
let result1 = [["teacher",[["Peter",[{"name":"Peter","profession":"teacher","age":20,"maritalStatus":"married"},
                     {"name":"Peter","profession":"teacher","age":20,"maritalStatus":"married"}]],["Michael",
                     [{"name":"Michael","profession":"teacher","age":50,"maritalStatus":"single"}]]]],
                ["scientific",[["Anna",[{"name":"Anna","profession":"scientific","age":20,"maritalStatus":"married"},
                      {"name":"Anna","profession":"scientific","age":20,"maritalStatus":"single"}]],["Rose",
                      [{"name":"Rose","profession":"scientific","age":50,"maritalStatus":"married"}]]]],
                ["politician",[["Anna",[{"name":"Anna","profession":"politician","age":50,"maritalStatus":"married"}]]]
                ]];
report(query().select().from(persons).groupBy(profession,name).execute(),JSON.stringify(result1));
...
let result2=[["teacher",[["Peter",[[20,[["married",[{"name":"Peter","profession":"teacher","age":20,"maritalStatus":"married"},
{"name":"Peter","profession":"teacher","age":20,"maritalStatus":"married"}]]]]]],["Michael",
[[50,[["single",[{"name":"Michael","profession":"teacher","age":50,"maritalStatus":"single"}]]]]]]]],
["scientific",[["Anna",[[20,[["married",[{"name":"Anna","profession":"scientific","age":20,"maritalStatus":"married"}]],
["single",[{"name":"Anna","profession":"scientific","age":20,"maritalStatus":"single"}]]]]]],
["Rose",[[50,[["married",[{"name":"Rose","profession":"scientific","age":50,"maritalStatus":"married"}]]]]]]]],
["politician",[["Anna",[[50,[["married",[{"name":"Anna","profession":"politician","age":50,"maritalStatus":"married"}]]]]]]]]];

//SELECT*FROMpersonsWHEREprofession='teacher'GROUPBYprofession,name,age
report(query().select().from(persons).groupBy(profession,name,age,maritalStatus).execute(),JSON.stringify(result2))
...
report(query().select(professionCount).from(persons).groupBy(profession).execute(),'[["teacher",3],["scientific",3],["politician",1]]')
...
report(query().select(professionCount).from(persons).groupBy(profession).orderBy(naturalCompare).execute(),'[["politician",1],["scientific",3],["teacher",3]]')
...
//SELECT*FROMnumbers
report(query().select().from(numbers).execute(),'[1,2,3,4,5,6,7,8,9]')

//SELECT*FROMnumbersGROUPBYparity
report(query().select().from(numbers).groupBy(parity).execute(),'[["odd",[1,3,5,7,9]],["even",[2,4,6,8]]]')
//,[["odd",[1,3,5,7,9]],["even",[2,4,6,8]]]);

//SELECT*FROMnumbersGROUPBYparity,isPrime
report(query().select().from(numbers).groupBy(parity,prime).execute(),'[["odd",[["divisible",[1,9]],["prime",[3,5,7]]]],["even",[["prime",[2]],["divisible",[4,6,8]]]]]')
...
report(query().select().from(numbers).groupBy(parity).having(odd).execute(),'[["odd",[1,3,5,7,9]]]')
//,[["odd",[1,3,5,7,9]]]);
...
report(query().select().from(numbers).orderBy(descendentCompare).execute(),'[9,8,7,6,5,4,3,2,1]')
//,[9,8,7,6,5,4,3,2,1]);
...
report(query().select().from(numbers).where(lessThan3,greaterThan4).execute(),'[1,2,5,6,7,8,9]')
//,[1,2,5,6,7,8,9]);
...
report(query().select(sumValues).from(persons).orderBy(naturalCompare).groupBy(nameGrouping).execute(),'[["Anna",4],["Michael",10],["Peter",10]]')
...
report(query().select().from(numbers1,numbers2).execute(),'[[1,4],[1,5],[2,4],[2,5]]')
//,[[1,4],[1,5],[2,4],[2,5]]);
...
report(query().select(student).from(teachers,students).where(teacherJoin).where(tutor1).execute(),'[{"studentName":"Michael","teacherName":"Peter"}]')
